...
public class VersionAwareMap implements Map<VersionedName, ClassInstance> {
    private final Multimap<String, ClassInstance> map;
     ...
     *
     * @param key the versioned name of {@code ClassInstance}
    ...
    public ClassInstance remove(Object key) {
        ...
        if (key instanceof VersionedName) {
            String name = ((VersionedName) key).getName();
            map.get(name).removeIf(cls -> cls.getVersionedName().equals(key));
        }
     ...
     * of calling {@link #put(VersionedName, ClassInstance) put(k, v)} on this map once
